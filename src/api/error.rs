...
pub enum MyError {
    ...
    // 读物fastq错误
    #[error("Error - Read fastq {file}: {error}")]
    ReadFastqError{file: String, error: anyhow::Error},

    // 保存fastq错误
    #[error("Error - Write fastq {file}: {error}")]
    WriteFastqError{file: String, error: anyhow::Error},
